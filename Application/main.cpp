...
|___________________________________________________________________*/
  ...
  Sound s_song, s_footsteps, s_origin,s_shot,s_death,s_congrats;
  
  s_song = snd_LoadSound("WAV\\origin.wav", snd_CONTROL_VOLUME, 0);
  s_footsteps= snd_LoadSound("WAV\\footsteps.wav", snd_CONTROL_VOLUME, 0);
  s_shot = snd_LoadSound("WAV\\RayGun.wav", snd_CONTROL_VOLUME, 0);
  s_death = snd_LoadSound("WAV\\zombieDeath.wav", snd_CONTROL_VOLUME, 0);
  s_congrats = snd_LoadSound("WAV\\song1.wav", snd_CONTROL_VOLUME, 0);
  //s_origin = snd_LoadSound("WAV\\origin.wav", snd_CONTROL_3D, 0);
...
|___________________________________________________________________*/
 ...
 /* gx3dObject *obj_pyra;
  gx3d_ReadLWO2File("Objects\\pyramid.lwo", &obj_pyra, gx3d_VERTEXFORMAT_DEFAULT, gx3d_DONT_LOAD_TEXTURES);
  gx3dTexture tex_brick = gx3d_InitTexture_File("Objects\\brick.bmp.bmp", 0, 0);*/
 ...
 gx3dObject *obj_grass;
  ...
  gx3dObject *obj_zombie;
  gx3d_ReadLWO2File("Objects\\billboard_ghost.lwo", &obj_zombie, gx3d_VERTEXFORMAT_DEFAULT, gx3d_DONT_LOAD_TEXTURES);
  gx3d_MakeDoubleSidedObject(obj_zombie);
  gx3dTexture tex_zombie = gx3d_InitTexture_File("Objects\\Images\\zombie.bmp", "Objects\\Images\\zombie_fa.bmp", 0);
  gx3dTexture tex_dead_zombie = gx3d_InitTexture_File("Objects\\Images\\deadZombie.bmp", "Objects\\Images\\zombie_fa.bmp", 0);
  
  gx3dObject *obj_oof;
  gx3d_ReadLWO2File("Objects\\billboard_oof.lwo", &obj_oof, gx3d_VERTEXFORMAT_DEFAULT, gx3d_DONT_LOAD_TEXTURES);
  //gx3d_MakeDoubleSidedObject(obj_oof);
  gx3dTexture tex_oof = gx3d_InitTexture_File("Objects\\Images\\OOF.bmp", "Objects\\Images\\OOF_fa.bmp", 0);

  const int NUM_ZOMBIES = 20;
  boolean zombieDraw[NUM_ZOMBIES];
  gx3dVector zombiePosition[NUM_ZOMBIES];
  float zombieSpeed[NUM_ZOMBIES];
  gx3dSphere zombieSphere[NUM_ZOMBIES];
  boolean zombieOnScreen[NUM_ZOMBIES];

  int zombieKill = 0;
  int zombieIcon = 0;
  const int MAX_OOF = 20;
  gx3dVector oofPos[MAX_OOF];
  int oofTimer[MAX_OOF];
  int oofIndx = 0;
  int MAX_HITS = 3;
  int numHits [NUM_ZOMBIES];
  ...
  for (int i = 0; i < NUM_ZOMBIES; i++) {
	  zombiePosition[i].x = (rand() % 100) - 50;
	  zombiePosition[i].y = 0;
	  zombiePosition[i].z = (rand() % 100) - 50;
	  zombieSpeed[i] = (((float)rand()/RAND_MAX)*0.09 + 0.01)/6;
	  zombieDraw[i] = true;
  }
...
|___________________________________________________________________*/
	...
	light_data.point.src.y = 5;  // 5 feet as requested
	light_data.point.src.z = -10;
...
|___________________________________________________________________*/
	...
	snd_SetSoundVolume(s_congrats, 85);
	snd_PlaySound(s_song, 1);
	snd_SetSoundVolume(s_song, 80);
	snd_SetSoundVolume(s_footsteps, 90);
	snd_SetSoundVolume(s_death, 88);
	

	//snd_SetSoundMode(s_origin, snd_3D_MODE_ORIGIN_RELATIVE, snd_3D_APPLY_NOW);
	//snd_SetSoundPosition(s_origin, 0, 0, 0, snd_3D_APPLY_NOW);
	//snd_SetSoundMinDistance(s_origin, 10, snd_3D_APPLY_NOW);
	//snd_SetSoundMaxDistance(s_origin, 100, snd_3D_APPLY_NOW);
	//snd_PlaySound(s_origin, 1);
  ...
  unsigned elapsed_time, last_time, new_time;
	...
	int lightMode = 0; //0 for ambi, 1 for dir, 2 for point
	...
	int grassz[200];
	for (int z = 0; z < 200; z++) {
		grassx[z] = (rand() % 100) - 50;
		grassz[z] = (rand() % 100) - 50;
	}
        ...
        if (event.keycode == 'w') 
          ...
          cmd_move &= ~(POSITION_MOVE_RIGHT);
			...
			//mouse left
				...
				for (int i = 0; i < NUM_ZOMBIES; i++) {
					if (zombieOnScreen[i]) {
						gxRelation rel = gx3d_Relation_Ray_Sphere(&viewVector, &zombieSphere[i]);
						if (rel != gxRELATION_OUTSIDE) {
							numHits[i]++;

							oofPos[oofIndx] = zombieSphere[i].center;
							oofTimer[oofIndx] = 1000 + elapsed_time;
							oofIndx = (oofIndx + 1) % MAX_OOF;

							if (numHits[i] == MAX_HITS) {
    ...
    if (gx3d_BeginRender ()) {                                             
	  ...
	  */
	  ...
	  gx3d_EnableLight(dir_light);
	   ...
	   //Draw random trees
			for (int i = 0; i < 5; i++) {
				...
				relation = gx3d_Relation_Box_Frustum(&box, &m);
				if (relation != gxRELATION_OUTSIDE) {
					gx3d_GetTranslateMatrix(&m, treex[i], 0, treez[i]);
	  ...
	  for (int i = 0; i < 200; i++) {
		  ...
		  sphere.center.x += grassx[i];
		  sphere.center.z += grassz[i];
		  relation = gx3d_Relation_Sphere_Frustum(&sphere);
		  if (relation != gxRELATION_OUTSIDE) {
			  gx3d_GetTranslateMatrix(&m, grassx[i], 0, grassz[i]);
	  ...
	  for (int i = 0; i < NUM_ZOMBIES; i++) {
		  zombiePosition[i].x += zombieSpeed[i];
		  zombiePosition[i].z += (zombieSpeed[i]*1.2f);
		  if (zombiePosition[i].x > 50)
			  zombieSpeed[i] *= -1;
		  else if (zombiePosition[i].x < -50) 
			  zombieSpeed[i] *= -1;
		  if (zombiePosition[i].z > 50)
			  zombieSpeed[i] *= -1;
		  else if (zombiePosition[i].z < -50)
			  zombieSpeed[i] *= -1;
	  ...
	  for (int i = 0; i < NUM_ZOMBIES; i++) {
		  if (zombieDraw[i]) {
			  zombieSphere[i] = obj_zombie->bound_sphere;
			  zombieSphere[i].center.x *= 4;
			 // zombieSphere[i].center.y *= 4;//
			  zombieSphere[i].center.z *= 4;

			  zombieSphere[i].center.x = zombiePosition[i].x;
			  zombieSphere[i].center.y = zombiePosition[i].y;
			  zombieSphere[i].center.z = zombiePosition[i].z;

			  zombieSphere[i].radius *= 4;
		  }
	  }
	  ...
	  gx3d_SetAmbientLight(color3d_white);
		  ...
		  for (int i = 0; i < NUM_ZOMBIES; i++) {
			  zombieOnScreen[i] = false;
			  gxRelation relation;
			  gx3dSphere sphere;
			  sphere = obj_zombie->bound_sphere;
			  sphere.center.x += zombiePosition[i].x;
			  sphere.center.y += zombiePosition[i].y;
			  sphere.center.z += zombiePosition[i].z;
			  if (zombieDraw[i]) {
				  relation = gx3d_Relation_Sphere_Frustum(&sphere);
				  if (relation != gxRELATION_OUTSIDE) {
					  gx3d_GetScaleMatrix(&m1, 4, 4, 4);
					  gx3d_GetBillboardRotateYMatrix(&m2, &billboard_normal, &heading);
					  gx3d_GetTranslateMatrix(&m3, zombiePosition[i].x, zombiePosition[i].y, zombiePosition[i].z);
					  gx3d_MultiplyMatrix(&m1, &m2, &m);
					  gx3d_MultiplyMatrix(&m1, &m3, &m);
					  gx3d_SetObjectMatrix(obj_zombie, &m);
					  gx3d_SetTexture(0, tex_zombie);
					  gx3d_DrawObject(obj_zombie, 0);
					  zombieOnScreen[i]=true;
				  }
			  }
	  }
	
		  const float OOF_SCALE = 3;
		  for (int i = 0; i < MAX_OOF; i++) {
			  if (oofTimer[i] > 0) {
				  oofTimer[i] -= elapsed_time;
				  gx3d_GetScaleMatrix(&m1, OOF_SCALE, OOF_SCALE, OOF_SCALE);
				  gx3d_GetBillboardRotateYMatrix(&m2, &billboard_normal, &heading);
				  float y = oofPos[i].y + (1 - (oofTimer[i] / 1000.0f))*(2 * OOF_SCALE);
				  gx3d_GetTranslateMatrix(&m3, oofPos[i].x, y + 6, oofPos[i].z);
				  gx3d_MultiplyMatrix(&m1, &m2, &m);
				  gx3d_MultiplyMatrix(&m, &m3, &m);
				  gx3d_SetObjectMatrix(obj_oof, &m);
				  gx3d_SetTexture(0, tex_oof);
				  gx3d_DrawObject(obj_oof, 0);
			  }
		  }
      ...
      // Disable alpha blending
		...
		if(zombieIcon) {
			gx3d_DisableZBuffer();
			gx3d_EnableAlphaBlending();
			for (int i = 0; i < zombieIcon; i++) {
				gx3d_GetScaleMatrix(&m1, 0.03f, 0.03f, 0.03f);
				gx3d_GetRotateYMatrix(&m2, 0);
				gx3d_GetTranslateMatrix(&m3, -0.5 + (0.05*i), 0.25, 0);
				gx3d_MultiplyMatrix(&m1, &m2, &m);
				gx3d_MultiplyMatrix(&m, &m3, &m);
				gx3d_SetObjectMatrix(obj_zombie, &m);
				gx3d_SetTexture(0, tex_dead_zombie);
				gx3d_DrawObject(obj_zombie, 0);
			}
			gx3d_DisableAlphaBlending();
			gx3d_EnableZBuffer();
